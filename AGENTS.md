...
0. **생태계·다레포·harness:** `[memory/core/ecosystem-contract.yaml](memory/core/ecosystem-contract.yaml)` + `[memory/core/inheritance-registry.yaml](memory/core/inheritance-registry.yaml)` — **status=active** (enforce ON).
