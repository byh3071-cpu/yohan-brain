...
| (대기) | **안 해도 됨** | RSS ingest — 배치가 24h 가드로 자동 (`memory/ingest/rss/`) |
...
| RSS ingest | 배치 내 24h 가드 — `memory/metrics/automation/last-rss-ingest.json` 기준 |
| git pull (부팅) | `%USERPROFILE%\git-auto-pull.ps1` (커스텀, Workspace 전체) |
| 텔레그램 완료 알림 | 배치 실행 시 1회 = 하루 최대 2회 (review/failed는 즉시) |

**수동(선택):** RSS 즉시 실행 `npm run ingest:all` 또는 `launch\ingest-rss.bat` (배치 가드와 무관, 멱등).
